...
function muestraAccidentesDistritosHora() {
    setInactive();
    $("#horaNav").addClass("active");
    hideFechas();
    showSelector();
    showMap();
    switch(Number($("select").val())){
        case 0: set00();break;
        case 1: set01();break;
        case 2: set02();break;
        case 3: set03();break;
...
function loadHoras() {
    $.ajax({
        url: "https://ec2-52-47-87-168.eu-west-3.compute.amazonaws.com/accidentesDistritosHoras",
        type: "GET",
        headers: {
            'Access-Control-Allow-Origin': '*'
        },
...
function initMap(datos) {
    ...
    if (datos[0].hora) {
        ...
        colors["fuencarral-el pardo"] = colors["fuencarral-elpardo"];
        colors["ciudad lineal"] = colors["ciudadlineal"];
        colors["san blas"] = colors["sanblas"];
        accidentes["fuencarral-el pardo"] = accidentes["fuencarral-elpardo"];
        accidentes["ciudad lineal"] = accidentes["ciudadlineal"];
        accidentes["san blas"] = accidentes["sanblas"];
    }
    else {
        accidentes = setArray(datos);
        colors = setColors(datos);
...
function setColors(datos) {
    let res = new Object();
    let coloreado = new Object();
    datos.forEach(elem => {
        if (elem.hora && elem.hora === hora) {
            if(max >= 5){
                if (max / 5 >= elem.accidentes)
                    res[elem.lugar.toLowerCase()] = "#FFE2A7";
                else if (max / 4 >= elem.accidentes)
                    res[elem.lugar.toLowerCase()] = "#FFC418";
                else if (max / 3 >= elem.accidentes)
                    res[elem.lugar.toLowerCase()] = "#FF9031";
                else if (max / 2 >= elem.accidentes)
                    res[elem.lugar.toLowerCase()] = "#FF1E00";
                else
                    res[elem.lugar.toLowerCase()] = "#400800";
            }
            else
                res[elem.lugar.toLowerCase()] = "#400800";
            coloreado[elem.lugar.toLowerCase()] = true;
        }
        else if(!elem.hora) {
            if (max / 5 >= elem.accidentes)
                res[elem.lugar.toLowerCase()] = "#FFE2A7";
        ...
        if(!coloreado[elem.lugar.toLowerCase()]){
            res[elem.lugar.toLowerCase()] = "#FFFFFF";
        }
    });
    return res;
}
